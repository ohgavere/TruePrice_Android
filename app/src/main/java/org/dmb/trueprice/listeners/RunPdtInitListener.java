...
public final class RunPdtInitListener implements View.OnClickListener, OnFocusChangeListener, TextWatcher
//, OnItemClickListener 
{
	
	private static final String logHead = RunPdtInitListener.class.getSimpleName();
	
	private Context initiator ;
//	Class<Login>
	
	public RunPdtInitListener( Context ctx ) {
		initiator = ctx ;
	}
...
//					break;
	...
	private void setLastFocusedViewId(int lastFocusedViewId) {
		RunPdtInitListener.lastFocusedViewId = lastFocusedViewId;
	}
...
//		}
    ...
    private TargetView final_TARGET = TargetView.MEASURE ;

    public void switchCalcTarget(){
        if (final_TARGET == TargetView.MEASURE)     final_TARGET = TargetView.SALE ;
        if (final_TARGET == TargetView.SALE)        final_TARGET = TargetView.MEASURE ;
    }
 ...
 */
    ...
    int currentFocusedView ;

	@Override
	public void afterTextChanged(Editable s) {

        currentFocusedView = RunPdtInitListener.getLastFocusedViewId() ;
...
//		);
		...
		if (// premier tour
			...
			//OU
			||
			// que la nouvelle valeur est plus/moins longue
